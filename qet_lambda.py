...
class EpsilonController:
    def __init__(self, init_eps: float, min_eps: float,
...
class Agent:
    def __init__(
        ...
        self.epsiloncontroller = EpsilonController(init_eps, min_eps,
                                                   epsilon_decay)
        ...
        self.beta = beta
        self.trace = dict()
        self.exp_trace_param = torch.rand((output_size, input_size),
                                          requires_grad=True)
        self.reset_trace()
    ...
    def train(self) -> float:
        states, actions, rewards, next_states, terminal_states = \
            self.replaybuffer.sample().values()
...
if __name__ == "__main__":
    ...
    agent = Agent(
        env.observation_space.shape[0], env.action_space.n, 0.001,
        0.99, 0.5, 0.5, 0.001, 10000, 32, 1.0, 0.001, 500.0
    )
    episodes = 150
